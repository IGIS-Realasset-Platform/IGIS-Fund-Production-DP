...
- **마이크로 인터랙션**: 모든 버튼과 상호작용 가능한 요소에는 부드러운 `hover` 효과(예: `transition-colors`, `hover:bg-[#333]`)를 적용하고, 마우스 오버 시 반드시 손모양 커서(`cursor: pointer`)가 나타나도록 설정합니다.
...
## 5. Git 커밋 의무화 (Mandatory Git Commit)

> [!IMPORTANT]
> **모든 코드 수정 및 피드백 반영 완료 시 '절대로 잊지 말고 즉시' Git 커밋을 수행해야 합니다.**
>
> - **건망증 금지**: 작업을 완료하거나 유저의 피드백을 반영한 직후에는 어물쩡 다음 단계로 넘어가지 말고 **반드시 즉시** `git add` 및 상세하고 명확한 커밋 메시지로 `git commit`을 완료해야 합니다.
> - **커밋 주기**: 파일 수정, 레이아웃 미세 조정, 버그 수정 등 유의미한 단위의 변경이 발생할 때마다 매번 잊지 않고 커밋을 수행하여 작업 이력을 투명하게 보존합니다.
> - **어시스턴트 행동 강령**: 유저가 지적하기 전에 스스로 커밋 상태를 점검하고 먼저 커밋을 제안하거나 실행해야 합니다.
