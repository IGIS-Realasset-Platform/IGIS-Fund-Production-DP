            ...
            <div className="w-[calc(100%-48px)] md:w-[calc(100%-100px)] max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-[720px_1fr] gap-10 lg:gap-16 items-center h-full pt-[60px] md:pt-0">
                ...
                <div className={`relative z-20 w-full max-w-[720px] -mt-[30px] lg:-mt-[110px] transition-all duration-[1200ms] ease-[cubic-bezier(0.16,1,0.3,1)] transform ${step >= 2 ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-12'}`}>
                    ...
                    <div className={`flex flex-col gap-6 md:gap-7 transition-all duration-[1200ms] ease-[cubic-bezier(0.16,1,0.3,1)] transform ${step >= 3 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                        <p className="text-[19px] md:text-[22px] font-bold text-black leading-[1.5] break-keep">
                            {lang === 'kr' ? (
                        ...
                        <p className="text-[19px] md:text-[21px] font-medium text-[#86868b] leading-[1.6] break-keep">
                            {lang === 'kr' ? (
                                <>IFPDP는 실시간 데이터 취합을 통해 <span className="text-[#1e3a8a] font-bold">정상 범위의 공정은 자율 작동</span>시키고,<br/><span className="text-[#1e3a8a] font-bold">리스크 한도를 이탈한 프로젝트(Red Flag)와 </span><span className="whitespace-nowrap"><span className="text-[#1e3a8a] font-bold">최종 의사결정(Approval)</span> 안건만을</span><br/>리더십에게 공유합니다.</>
                            ) : (
