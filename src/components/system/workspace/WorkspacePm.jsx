...
export default function WorkspacePm({ part = 1 }) {
    ...
    const handleTargetTask = () => {
        ...
        console.log('[WorkspacePm] handleTargetTask 실행됨. targetTaskId:', targetTaskId, 'tasks.length:', tasks.length);
        if (targetTaskId && tasks.length > 0) {
            const targetTask = tasks.find(t => {
                console.log('[WorkspacePm] tasks 비교 중... t.id:', t.id, 'String(t.id):', String(t.id), 'String(targetTaskId):', String(targetTaskId));
                return String(t.id) === String(targetTaskId);
            });
            console.log('[WorkspacePm] targetTask 찾음 결과:', targetTask);
            if (targetTask) {
                setProjectShowAll(true);
                setExpandedTaskId(targetTask.id);
                console.log('[WorkspacePm] expandedTaskId 설정 완료:', targetTask.id);
                setTimeout(() => {
                    const el = document.getElementById(`task-${targetTask.id}`);
                    console.log('[WorkspacePm] DOM 엘리먼트 찾음 결과 (el):', el);
                    if (el) {
    ...
    useEffect(() => {
        console.log('[WorkspacePm] useEffect 실행됨. isLoadingTasks:', isLoadingTasks, 'tasks.length:', tasks.length);
    ...
    const handleSaveRow = async () => {
        ...
        try {
            if (editingTaskId) {
                ...
                const { data, error } = await supabase.from('iota_pm_tasks').insert([taskToSave]).select();
                if (error) throw error;
    ...
    const handleMoveTaskUp = async (index) => {
        ...
        try {
            ...
            await supabase.from('iota_pm_tasks').update({ created_at: newPrevTime }).eq('id', prev.id);
        } catch (e) {
    ...
    const handleMoveTaskDown = async (index) => {
        ...
        try {
            await supabase.from('iota_pm_tasks').update({ created_at: newCurrentTime }).eq('id', current.id);
            await supabase.from('iota_pm_tasks').update({ created_at: newNextTime }).eq('id', next.id);
        } catch (e) {
            localStorage.setItem('iota_pm_tasks_fallback', JSON.stringify(newTasks));
        }
    };
    ...
    return (
        <div className="w-full flex-1 flex flex-col pt-[28px] pb-[200px] max-w-[1200px] mx-auto">
            ...
            <div className="w-full flex justify-between items-center mb-[40px] gap-[40px]">
                ...
                {/* PM Team Structure */}
                <div className="border border-[#333] rounded-[24px] flex items-center bg-transparent overflow-x-auto hide-scrollbar max-w-[800px] pl-[20px] pr-[10px] py-[10px]">
                            ...
                            <tr key={idx} className="hover:bg-[#292928] transition-colors">
                                ...
                                <td className="px-[24px] py-[16px] text-center">
                                    <div className="inline-flex items-center justify-center bg-black rounded-[12px] px-[12px] py-[6px]">
                                        <span className={`text-[13px] font-bold ${row.status === '주의' ? 'text-[#f59e0b]' : row.status === '정상' ? 'text-[#2997FF]' : 'text-[#86868B]'}`}>{row.status}</span>
                                    </div>
                                </td>
                            </tr>
                        ))}
