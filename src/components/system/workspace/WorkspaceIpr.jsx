...
export default function WorkspaceIpr() {
    ...
    const handleTargetTask = () => {
        ...
        console.log('[WorkspaceIpr] handleTargetTask 실행됨. targetTaskId:', targetTaskId, 'tasks.length:', tasks.length);
        if (targetTaskId && tasks.length > 0) {
            const targetTask = tasks.find(t => {
                console.log('[WorkspaceIpr] tasks 비교 중... t.id:', t.id, 'String(t.id):', String(t.id), 'String(targetTaskId):', String(targetTaskId));
                return String(t.id) === String(targetTaskId);
            });
            console.log('[WorkspaceIpr] targetTask 찾음 결과:', targetTask);
            if (targetTask) {
                setProjectShowAll(true);
                setExpandedTaskId(targetTask.id);
                console.log('[WorkspaceIpr] expandedTaskId 설정 완료:', targetTask.id);
                setTimeout(() => {
                    const el = document.getElementById(`task-${targetTask.id}`);
                    console.log('[WorkspaceIpr] DOM 엘리먼트 찾음 결과 (el):', el);
                    if (el) {
    ...
    useEffect(() => {
        console.log('[WorkspaceIpr] useEffect 실행됨. isLoadingTasks:', isLoadingTasks, 'tasks.length:', tasks.length);
    ...
    const handleMoveTaskUp = async (index) => {
        ...
        try {
            await supabase.from('iota_ipr_tasks').update({ created_at: newCurrentTime }).eq('id', current.id);
            await supabase.from('iota_ipr_tasks').update({ created_at: newPrevTime }).eq('id', prev.id);
        } catch (e) {
    ...
    const handleMoveTaskDown = async (index) => {
        ...
        try {
            await supabase.from('iota_ipr_tasks').update({ created_at: newCurrentTime }).eq('id', current.id);
            await supabase.from('iota_ipr_tasks').update({ created_at: newNextTime }).eq('id', next.id);
        } catch (e) {
            localStorage.setItem('iota_ipr_tasks_fallback', JSON.stringify(newTasks));
        }
    };
    ...
    return (
                <div className="w-full flex-1 flex flex-col pt-[28px] pb-[100px] max-w-[1200px] mx-auto">
