...
export default function WorkspaceDevelopment() {
    ...
    const handleMoveTaskUp = async (index) => {
        ...
        try {
            await executeWithTimeout(supabase.from('iota_development_tasks').update({ created_at: current.created_at }).eq('id', current.id));
            await executeWithTimeout(supabase.from('iota_development_tasks').update({ created_at: prev.created_at }).eq('id', prev.id));
        } catch (e) {
    ...
    const handleMoveTaskDown = async (index) => {
        ...
        try {
            await executeWithTimeout(supabase.from('iota_development_tasks').update({ created_at: current.created_at }).eq('id', current.id));
            await executeWithTimeout(supabase.from('iota_development_tasks').update({ created_at: next.created_at }).eq('id', next.id));
        } catch (e) {
            localStorage.setItem('iota_development_tasks_fallback', JSON.stringify(newTasks));
        }
    };
