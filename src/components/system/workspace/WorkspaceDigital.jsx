...
export default function WorkspaceDigital() {
    ...
    const handleMoveTaskUp = async (index) => {
        ...
        try {
            const { error: err1 } = await executeWithTimeout(supabase.from('iota_digital_tasks').update({ created_at: newCurrentTime }).eq('id', current.id));
            const { error: err2 } = await executeWithTimeout(supabase.from('iota_digital_tasks').update({ created_at: newPrevTime }).eq('id', prev.id));
            if (err1 || err2) {
    ...
    const handleMoveTaskDown = async (index) => {
        ...
        try {
            const { error: err1 } = await executeWithTimeout(supabase.from('iota_digital_tasks').update({ created_at: newCurrentTime }).eq('id', current.id));
            const { error: err2 } = await executeWithTimeout(supabase.from('iota_digital_tasks').update({ created_at: newNextTime }).eq('id', nextTask.id));
            if (err1 || err2) {
                console.error('Reorder update failed:', err1 || err2);
                alert('순서 변경 실패: 권한이 없거나 서버 오류입니다.');
            }
