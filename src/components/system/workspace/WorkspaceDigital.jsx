...
export default function WorkspaceDigital() {
    ...
    const handleMoveTaskUp = async (index) => {
        ...
        try {
            const { data: d1, error: err1 } = await executeWithTimeout(supabase.from('iota_digital_tasks').update({ created_at: newCurrentTime }).eq('id', current.id).select());
            const { data: d2, error: err2 } = await executeWithTimeout(supabase.from('iota_digital_tasks').update({ created_at: newPrevTime }).eq('id', prev.id).select());
            if (err1 || err2) {
    ...
    const handleMoveTaskDown = async (index) => {
        ...
        try {
            const { data: d1, error: err1 } = await executeWithTimeout(supabase.from('iota_digital_tasks').update({ created_at: newCurrentTime }).eq('id', current.id).select());
            const { data: d2, error: err2 } = await executeWithTimeout(supabase.from('iota_digital_tasks').update({ created_at: newNextTime }).eq('id', nextTask.id).select());
            if (err1 || err2) {
                console.error('Reorder update failed:', err1 || err2);
                alert('순서 변경 실패: 권한이 없거나 서버 오류입니다.');
            }
                    ...
                    <div className="flex flex-col gap-[8px]">
                        <AnimatePresence>
                            ...
                            <motion.div 
                                ...
                                className={`scroll-mt-[100px] w-full relative rounded-[24px] px-6 pt-[18px] pb-[18px] cursor-pointer transition-all duration-300 group/row ${(expandedTaskId === 'ALL' || expandedTaskId === row.id) ? 'border-[2px] border-transparent [background:linear-gradient(#272726,#272726)_padding-box,linear-gradient(to_bottom_right,#d6efe9,#82afb9,#4c6e86)_border-box]' : 'bg-[#272726] border border-[#3c3c3c] hover:bg-[#333]'}`}
                            >
                            {/* 삭제 및 정렬 버튼 */}
                            {isAuthorized && (
