import React from 'react';

export default function IotaOne427() {
    return (
        <div className="w-[1200px] mx-auto flex-1 flex flex-col pt-[60px] shrink-0 pb-[60px]">
            <div className="w-full flex justify-between items-end mb-[24px]">
                <h1 className="text-[36px] font-bold text-[#1D1D1F] dark:text-white tracking-tight leading-none font-['Inter'] -translate-y-[1px] translate-x-[4px]">IOTA Seoul 1 427</h1>
            </div>
            
            <div className="w-full flex-1 flex flex-col items-center justify-center bg-white dark:bg-[#1C1C1E] rounded-[32px] border border-black/10 dark:border-[#2C2C2E] shadow-sm min-h-[400px]">
                <div className="w-16 h-16 mb-6 text-[#86868B]">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
                    </svg>
                </div>
                <h2 className="text-[24px] font-bold tracking-tight text-[#1D1D1F] dark:text-[#E5E5E5]">IOTA One 427</h2>
                <p className="mt-2 text-[15px] text-[#86868B] dark:text-[#A1A1AA]">데이터 및 컴포넌트 뼈대 구조화 대기 중</p>
            </div>
        </div>
    );
}
