...
export default function IotaOne427() {
        const navigate = (path) => {
        ...
        const fetchData = async () => {
            try {
                const [marketingRes, specsRes, researchRes, comparisonRes, capitalRes, historyRes, tasksRes] = await Promise.all([
                    ...
                    supabase.from('iota_capital_stack').select('*').eq('vehicle_name', '427').abortSignal(controller.signal),
                    supabase.from('iota_project_history').select('*').eq('vehicle_name', '427').order('created_at', { ascending: true }).abortSignal(controller.signal),
                ...
                if (capitalRes.data) {
                    ...
                    capitalRes.data.forEach(item => {
                        ...
                        // 427 special rule: merge '1종 종류주', '보통주', '주주대여금' into 'Equity'
                        ...
                        // 427 special rule: merge 'Tr.A-2' into 'Tr.A-1'
                        if (tranche === 'Tr.A-2') {
                            tranche = 'Tr.A-1';
                            sortOrder = 1;
                        }
                ...
                if (controller.signal.aborted) return;
                console.error("Error fetching IotaOne427 data:", err);
            }
        };
    ...
    return (
                ...
                <div className="w-full flex justify-between items-start mb-[22px]">
                    <h1 className="text-[32px] font-bold text-white tracking-tight leading-none font-['Inter'] translate-y-[13px] translate-x-[4px]">IOTA Seoul One 427</h1>
                    ...
                    <div className="flex items-center h-[48px] translate-y-[4px] -translate-x-[30px]">
                        ...
                        <div className="flex flex-col items-center justify-center h-full px-[20px]">
                            ...
                            <span className="text-[18px] font-bold text-[#1D1D1F] dark:text-[#A1A1AA] tracking-tight font-['Inter']">Office, Hotel</span>
                        </div>
                ...
                <div className="w-full bg-[#292928] border border-[#3c3c3c] rounded-[24px] h-[74px] flex items-center justify-between px-8">
                    <div className="flex items-center text-[16px]">
                        ...
                        <span className="text-white font-bold">2025년 4월 1차(철거)PF 완료</span>
                        <span className="text-[#666] mx-[14px]">ㅣ</span>
                        <span className="text-white font-bold">철거공사 진행중</span>
                        <span className="text-[#666] mx-[14px]">ㅣ</span>
                        <span className="text-white font-bold">2027년 5월 착공 예정</span>
                        <span className="text-[#666] mx-[14px]">ㅣ</span>
                ...
                <div className="w-[1230px] -ml-[15px] h-[112px] mt-[9px] mb-[13px] relative group cursor-pointer rounded-[24px] hover:bg-[#242424] transition-colors duration-300 px-[40px]">
                    <div className="absolute top-[56px] left-[40px] right-[40px] h-px bg-[#444] group-hover:bg-[#E5E5E5] transition-colors duration-300 z-0">
                        {[
                            { date: '2021.11', label: 'PFV설립', left: 0 },
                            { date: '2022.02', label: '자산매입', left: 0.062 },
                            { date: '2024.02', label: '정비계획변경', left: 0.174 },
                            { date: '2024.05', label: '1차연장', left: 0.243 },
                            { date: '2024.09', label: '통합심의完', left: 0.305 },
                            { date: '2024.12', label: '사업시행인가完', left: 0.38 },
                            { date: '2025.01', label: '2차연장', left: 0.45 },
                            { date: '2025.02', label: '관리처분인가完', left: 0.52 },
                            { date: '2025.05', label: '1차PF/철거시작', left: 0.61 },
                            { date: 'NOW', label: '', type: 'now', left: 0.66 },
                            { date: '2026.11', label: '통합PF', left: 0.74 },
                            { date: '2027.05', label: 'IOTA1 착공', left: 0.82 },
                            { date: '2028.06', label: 'IOTA2 착공', left: 0.91 },
                ...
                <div 
                    ...
                    className={`w-full cursor-pointer bg-[#292928] border border-[#3c3c3c] hover:border-[#666] rounded-[32px] pl-[24px] pr-[32px] flex flex-col mb-[20px] transition-all duration-300 ${isIssuesExpanded ? 'pt-[24px] pb-[32px]' : 'pt-[24px] pb-[34px] min-h-[127px] h-auto'} -mt-[8px]`}
                >
                    ...
                    {!isIssuesExpanded ? (
                        <div className="flex flex-col gap-[4px] mt-[4px] animate-fadeIn">
                            <span className="text-[15px] font-medium text-[#c3c2b7] leading-[22px] tracking-tight">
                                <span className="font-bold text-white mr-1">브릿지 상환 완료:</span>
                                단기 연장 만기 도래 전 총 2조 2,000억원 규모의 1차 PF를 조달 완료하며 브릿지론 청산.
                            </span>
                            <span className="text-[15px] font-medium text-[#c3c2b7] leading-[22px] tracking-tight mt-[2px]">
                                <span className="font-bold text-white mr-1">철거 단계 전용 PF:</span>
                                철거 및 착공 전 필수사업비 조달을 목적으로 설계된 24개월 만기 구조. (2027년 5월 본공사 착공 시점에 건축비를 조달하기 위한 2차 PF로 대환 예정)
                            </span>
                            <span className="text-[15px] font-medium text-[#c3c2b7] leading-[22px] tracking-tight mt-[2px]">
                                <span className="font-bold text-white mr-1">통합 PF 전향:</span>
                                초대형 통합개발 통한 희소성 극대화 및 독보적 자산가치 구축을 목표로 IOTA 2 816프로젝트와 통합 PF 방안 검토 중.
                            </span>
                        </div>
                    ) : (
                        <div className="flex flex-col gap-[28px] mt-[14px] animate-fadeIn">
                            ...
                            <div className="flex flex-col gap-[8px]">
                                <div className="text-[15px] font-bold text-[#E5E5E5] tracking-tight">Phase 1. PFV 설립 <span className="text-[#86868B] font-medium ml-1">(2021년 11월)</span></div>
                                <div className="flex flex-col gap-[4px]">
                                    <div className="text-[14px] text-[#A1A1AA] leading-[22px] tracking-tight">
                                        <span className="font-semibold text-[#c3c2b7] mr-1">사업 개요:</span> 
                                        밀레니엄힐튼호텔 부지(6,156평)를 1조 1,000억원에 매입, 기존 호텔을 철거하고 지하 9층~지상 20층 규모의 복합 오피스를 개발하는 구조.
                                    </div>
                                    <div className="text-[14px] text-[#A1A1AA] leading-[22px] tracking-tight">
                                        <span className="font-semibold text-[#c3c2b7] mr-1">지출 비용:</span> 
                                        힐튼호텔의 기존 인력에 대한 Outplacement(위로금), 상생기금, M/L 운영손실 보전 등 실질적인 매입부대비용이 가중되어 실제 자산매입 비용은 1조 4,519억원.
                                    </div>
                                    <div className="text-[14px] text-[#A1A1AA] leading-[22px] tracking-tight">
                                        <span className="font-semibold text-[#c3c2b7] mr-1">공사 및 금융:</span> 
                                        본공사비는 당시 시장 기준인 평당 780만원, 철거비는 평당 40만원으로 책정. 저금리 환경 속에서 총 6,172억원의 금융비용으로 68개월(인허가 27M, 본공사 41M) 만에 준공하는 구조였음.
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* Phase 2 */}
                            <div className="flex flex-col gap-[8px]">
                                <div className="text-[15px] font-bold text-[#E5E5E5] tracking-tight">Phase 2. 1차 연장 / 리파이낸싱 <span className="text-[#86868B] font-medium ml-1">(2024년 4~5월)</span></div>
                                <div className="flex flex-col gap-[4px]">
                                    <div className="text-[14px] text-[#A1A1AA] leading-[22px] tracking-tight">
                                        <span className="font-semibold text-[#c3c2b7] mr-1">정비기본계획 변경 반영:</span> 
                                        2023년 2월 서울시의 2030 도시·주거환경정비기본계획 발표에 맞추어 개방형 녹지 조성 인센티브를 받기 위해 인허가 방향 선회.
                                    </div>
                                    <div className="text-[14px] text-[#A1A1AA] leading-[22px] tracking-tight">
                                        <span className="font-semibold text-[#c3c2b7] mr-1">사업성의 양적 확대:</span> 
                                        개발 연면적이 77,983평에서 103,422평으로 약 32.6% 급증. 상한 용적률이 1,042.14%로 극대화되었고 최고 높이도 지상 39층으로 확대. 매출이 4.49조원으로 치솟아, 상승한 원가 부담을 상쇄하며 개발이익이 5,712억원(이익률 12.7%)까지 오름.
                                    </div>
                                    <div className="text-[14px] text-[#A1A1AA] leading-[22px] tracking-tight">
                                        <span className="font-semibold text-[#c3c2b7] mr-1">주의점:</span> 
                                        당시 제시된 1.62조원 리파이낸싱 계획(TM)은 조달시장 악화로 실행되지 못했으며, 실제로는 1.44조원 브릿지론의 8개월 단기 연장(24.05.24 ➔ 25.01.24)을 체결.
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* Phase 3 */}
                            <div className="flex flex-col gap-[8px]">
                                <div className="text-[15px] font-bold text-[#E5E5E5] tracking-tight">Phase 3. 2차 연장 <span className="text-[#86868B] font-medium ml-1">(2025년 1월)</span></div>
                                <div className="flex flex-col gap-[4px]">
                                    <div className="text-[14px] text-[#A1A1AA] leading-[22px] tracking-tight">
                                        <span className="font-semibold text-[#c3c2b7] mr-1">대주단 금융 경색:</span> 
                                        리파이낸싱 완료 직전 기존 Tr.A 앵커였던 KB국민은행이 내부 사정으로 참여 불가를 선언하며 대규모 디폴트 위기 엄습.
                                    </div>
                                    <div className="text-[14px] text-[#A1A1AA] leading-[22px] tracking-tight">
                                        <span className="font-semibold text-[#c3c2b7] mr-1">긴급 연장 및 이지스 출자:</span> 
                                        대출을 대환하지 못하고 기존 브릿지론(1.44조원)을 7개월 추가 단기 연장(25.01.24 ➔ 25.08.25)하는 안으로 급우회. 대주단 이자 재원 보완을 위해 최대 240억원, 연단리 20% 이하 조건의 긴급 후순위 펀드(신종 수익증권 D종) 출자 확약을 제출하여 딜 클로징.
                                    </div>
                                    <div className="text-[14px] text-[#A1A1AA] leading-[22px] tracking-tight">
                                        <span className="font-semibold text-[#c3c2b7] mr-1">원가 현실화:</span> 
                                        평당 도급공사비가 1,139만원/평으로 인상 반영되어 총사업비가 4.6조원으로 불어났고, 개발이익은 매각보수 차감 후 기준 6.9%(3,486억원)로 하향 조정됨.
                                    </div>
                                    <div className="text-[14px] text-[#A1A1AA] leading-[22px] tracking-tight">
                                        <span className="font-semibold text-[#c3c2b7] mr-1">이지스 긴급 수혈 지원:</span> 
                                        대주단 이탈 위기 속에서 이지스가 자체 리스크를 감수하며 240억원의 고금리 후순위 출자 확약을 제공함으로써 디폴트를 원천 방어하고 본 PF로 가기 위한 안정적 발판을 구축함.
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* Phase 4 */}
                            <div className="flex flex-col gap-[8px]">
                                <div className="text-[15px] font-bold text-[#E5E5E5] tracking-tight">Phase 4. 1차 PF 및 철거</div>
                                <div className="flex flex-col gap-[4px]">
                                    <div className="text-[14px] text-[#A1A1AA] leading-[22px] tracking-tight">
                                        <span className="font-semibold text-[#c3c2b7] mr-1">브릿지 상환 완료:</span> 
                                        단기 연장 만기 도래 전 총 2조 2,000억원 규모의 1차 PF를 조달 완료하며 브릿지론 청산.
                                    </div>
                                    <div className="text-[14px] text-[#A1A1AA] leading-[22px] tracking-tight">
                                        <span className="font-semibold text-[#c3c2b7] mr-1">철거 단계 전용 PF:</span> 
                                        철거 및 착공 전 필수사업비 조달을 목적으로 설계된 24개월 만기 구조. (2027년 5월 본공사 착공 시점에 건축비를 조달하기 위한 2차 PF로 대환 예정)
                                    </div>
                                    <div className="text-[14px] text-[#A1A1AA] leading-[22px] tracking-tight">
                                        <span className="font-semibold text-[#c3c2b7] mr-1">통합 PF 전향:</span> 
                                        초대형 통합개발 통한 희소성 극대화 및 독보적 자산가치 구축을 목표로 IOTA 2 816프로젝트와 통합 PF 방안 검토 중.
                                    </div>
                                </div>
                            </div>
                ...
                <div className="w-full flex gap-[20px] mb-[20px]">
                    {/* Left Column (Image) */}
                    <div className="w-[450px] h-[452px] relative rounded-[32px] overflow-hidden group border border-[#3c3c3c]">
                        <img src={`${import.meta.env.BASE_URL}iota1.jpg`} alt="IOTA Seoul" className="w-full h-full object-cover transition-transform duration-700 ease-in-out group-hover:scale-[1.03]" />
                    ...
                    <div className="flex-1 bg-[#292928] border border-[#3c3c3c] rounded-[32px] pt-[24px] pb-[4px] relative flex flex-col h-[452px] overflow-hidden">
                        ...
                        {/* Top Header Row */}
                        <div className="flex items-center gap-[24px] mb-[24px] px-[28px]">
                            <div className="flex flex-col gap-[3px]">
                                <span className="text-[15px] font-bold text-[#86868B] tracking-tight">시공사</span>
                                <span className="text-[28px] font-bold text-white tracking-tighter leading-none">현대건설</span>
                            </div>
                            <div className="w-[1px] h-[36px] bg-[#444]/50"></div>
                            <div className="flex flex-col gap-[3px]">
                                <span className="text-[15px] font-bold text-[#86868B] tracking-tight">설계사</span>
                                <span className="text-[28px] font-bold text-white tracking-tighter leading-none">Foster + Partners</span>
                            </div>
                            <div className="w-[1px] h-[36px] bg-[#444]/50"></div>
                            <div className="flex flex-col gap-[3px]">
                                <span className="text-[15px] font-bold text-[#86868B] tracking-tight">바닥면적</span>
                                <span className="text-[28px] font-bold text-white tracking-tighter leading-none">1,149평</span>
                            </div>
                            <div className="w-[1px] h-[36px] bg-[#444]/50"></div>
                            <div className="flex flex-col gap-[3px]">
                                <span className="text-[15px] font-bold text-[#86868B] tracking-tight">전력</span>
                                <span className="text-[28px] font-bold text-white tracking-tighter leading-none">35MW</span>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 flex-1">
                            ...
                            <div className="grid grid-cols-[84px_1fr] items-center border-b border-r border-[#444]/50 pl-[28px] pr-[16px]">
                                <span className="text-[15px] font-bold text-[#86868B] tracking-tight shrink-0">시행법인</span>
                                <div className="relative group/tooltip min-w-0 flex items-center">
                                    <span className="text-[16px] text-[#E5E5E5] tracking-tight font-medium truncate pr-4 cursor-default w-full">와이디427피에프브이(주)</span>
                                    <div className="absolute bottom-[100%] left-0 mb-2 px-3 py-1.5 bg-[#333] text-[#E5E5E5] text-[12px] font-medium rounded-md whitespace-nowrap opacity-0 group-hover/tooltip:opacity-100 transition-opacity duration-200 pointer-events-none z-50 shadow-lg border border-[#444]">
                                        와이디427피에프브이(주)
                                    </div>
                                </div>
                            </div>
                            <div className="grid grid-cols-[84px_1fr] items-center border-b border-[#444]/50 pl-[24px] pr-[32px]">
                                <span className="text-[15px] font-bold text-[#86868B] tracking-tight shrink-0">규모</span>
                                <div className="relative group/tooltip min-w-0 flex items-center">
                                    <span className="text-[16px] text-[#E5E5E5] tracking-tight font-medium truncate cursor-default w-full">B10/34F(Office), B10/39F(Hotel)</span>
                                    <div className="absolute bottom-[100%] left-0 mb-2 px-3 py-1.5 bg-[#333] text-[#E5E5E5] text-[12px] font-medium rounded-md whitespace-nowrap opacity-0 group-hover/tooltip:opacity-100 transition-opacity duration-200 pointer-events-none z-50 shadow-lg border border-[#444]">
                                        B10/34F(Office), B10/39F(Hotel)
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* Row 2 */}
                            <div className="grid grid-cols-[84px_1fr] items-center border-b border-r border-[#444]/50 pl-[28px] pr-[16px]">
                                <span className="text-[15px] font-bold text-[#86868B] tracking-tight shrink-0">위치</span>
                                <div className="relative group/tooltip min-w-0 flex items-center">
                                    <span className="text-[16px] text-[#E5E5E5] tracking-tight font-medium truncate pr-4 cursor-default w-full">서울시 중구 남대문로5가 395번지 일원</span>
                                    <div className="absolute bottom-[100%] left-0 mb-2 px-3 py-1.5 bg-[#333] text-[#E5E5E5] text-[12px] font-medium rounded-md whitespace-nowrap opacity-0 group-hover/tooltip:opacity-100 transition-opacity duration-200 pointer-events-none z-50 shadow-lg border border-[#444]">
                                        서울시 중구 남대문로5가 395번지 일원
                                    </div>
                                </div>
                            </div>
                            <div className="grid grid-cols-[84px_1fr] items-center border-b border-[#444]/50 pl-[24px] pr-[32px]">
                                <span className="text-[15px] font-bold text-[#86868B] tracking-tight shrink-0">층고</span>
                                <div className="relative group/tooltip min-w-0 flex items-center">
                                    <span className="text-[16px] text-[#E5E5E5] tracking-tight font-medium truncate cursor-default w-full">4.4m(Office) / 3.3m(Hotel)</span>
                                    <div className="absolute bottom-[100%] left-0 mb-2 px-3 py-1.5 bg-[#333] text-[#E5E5E5] text-[12px] font-medium rounded-md whitespace-nowrap opacity-0 group-hover/tooltip:opacity-100 transition-opacity duration-200 pointer-events-none z-50 shadow-lg border border-[#444]">
                                        4.4m(Office) / 3.3m(Hotel)
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* Row 3 */}
                            <div className="grid grid-cols-[84px_1fr] items-center border-b border-r border-[#444]/50 pl-[28px] pr-[16px]">
                                <span className="text-[15px] font-bold text-[#86868B] tracking-tight shrink-0">연면적</span>
                                <div className="relative group/tooltip min-w-0 flex items-center">
                                    <span className="text-[15px] text-[#E5E5E5] tracking-tight font-medium truncate pr-4 cursor-default w-full">281,547m² (85,168평_Office) / 57,435m² (17,374평_Hotel)</span>
                                    <div className="absolute bottom-[100%] left-0 mb-2 px-3 py-1.5 bg-[#333] text-[#E5E5E5] text-[12px] font-medium rounded-md whitespace-nowrap opacity-0 group-hover/tooltip:opacity-100 transition-opacity duration-200 pointer-events-none z-50 shadow-lg border border-[#444]">
                                        281,547m² (85,168평_Office) / 57,435m² (17,374평_Hotel)
                                    </div>
                                </div>
                            </div>
                            <div className="grid grid-cols-[84px_1fr] items-center border-b border-[#444]/50 pl-[24px] pr-[32px]">
                                <span className="text-[15px] font-bold text-[#86868B] tracking-tight shrink-0">천정고</span>
                                <div className="relative group/tooltip min-w-0 flex items-center">
                                    <span className="text-[16px] text-[#E5E5E5] tracking-tight font-medium truncate cursor-default w-full">2.95m(Office) / 2.6m(Hotel)</span>
                                    <div className="absolute bottom-[100%] left-0 mb-2 px-3 py-1.5 bg-[#333] text-[#E5E5E5] text-[12px] font-medium rounded-md whitespace-nowrap opacity-0 group-hover/tooltip:opacity-100 transition-opacity duration-200 pointer-events-none z-50 shadow-lg border border-[#444]">
                                        2.95m(Office) / 2.6m(Hotel)
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* Row 4 */}
                            <div className="grid grid-cols-[84px_1fr] items-center border-b border-r border-[#444]/50 pl-[28px] pr-[16px]">
                                <span className="text-[15px] font-bold text-[#86868B] tracking-tight shrink-0">사업면적</span>
                                <div className="relative group/tooltip min-w-0 flex items-center">
                                    <span className="text-[16px] text-[#E5E5E5] tracking-tight font-medium truncate pr-4 cursor-default w-full">18,953m² (5,733평)</span>
                                    <div className="absolute bottom-[100%] left-0 mb-2 px-3 py-1.5 bg-[#333] text-[#E5E5E5] text-[12px] font-medium rounded-md whitespace-nowrap opacity-0 group-hover/tooltip:opacity-100 transition-opacity duration-200 pointer-events-none z-50 shadow-lg border border-[#444]">
                                        18,953m² (5,733평)
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-[84px_1fr] items-center border-b border-[#444]/50 pl-[24px] pr-[32px]">
                                <span className="text-[15px] font-bold text-[#86868B] tracking-tight shrink-0">주차대수</span>
                                <div className="relative group/tooltip min-w-0 flex items-center">
                                    <span className="text-[16px] text-[#E5E5E5] tracking-tight font-medium truncate cursor-default w-full">2,232 대</span>
                                    <div className="absolute bottom-[100%] left-0 mb-2 px-3 py-1.5 bg-[#333] text-[#E5E5E5] text-[12px] font-medium rounded-md whitespace-nowrap opacity-0 group-hover/tooltip:opacity-100 transition-opacity duration-200 pointer-events-none z-50 shadow-lg border border-[#444]">
                                        2,232 대
                                    </div>
                                </div>
                            ...
                            {/* Row 5 */}
                            <div className="grid grid-cols-[84px_1fr] items-center border-b border-r border-[#444]/50 pl-[28px] pr-[16px]">
                                <span className="text-[15px] font-bold text-[#86868B] tracking-tight shrink-0">용도</span>
                                <div className="relative group/tooltip min-w-0 flex items-center">
                                    <span className="text-[16px] text-[#E5E5E5] tracking-tight font-medium truncate pr-4 cursor-default w-full">업무시설, 숙박시설, 판매시설</span>
                                    <div className="absolute bottom-[100%] left-0 mb-2 px-3 py-1.5 bg-[#333] text-[#E5E5E5] text-[12px] font-medium rounded-md whitespace-nowrap opacity-0 group-hover/tooltip:opacity-100 transition-opacity duration-200 pointer-events-none z-50 shadow-lg border border-[#444]">
                                        업무시설, 숙박시설, 판매시설
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-[84px_1fr] items-center border-b border-[#444]/50 pl-[24px] pr-[32px]">
                                <span className="text-[15px] font-bold text-[#86868B] tracking-tight shrink-0">높이</span>
                                <div className="relative group/tooltip min-w-0 flex items-center">
                                    <span className="text-[16px] text-[#E5E5E5] tracking-tight font-medium truncate cursor-default w-full">141.8m</span>
                                    <div className="absolute bottom-[100%] left-0 mb-2 px-3 py-1.5 bg-[#333] text-[#E5E5E5] text-[12px] font-medium rounded-md whitespace-nowrap opacity-0 group-hover/tooltip:opacity-100 transition-opacity duration-200 pointer-events-none z-50 shadow-lg border border-[#444]">
                                        141.8m
                                    </div>
                                </div>
                            ...
                            {/* Row 6 */}
                            <div className="grid grid-cols-[84px_1fr] items-center border-b border-r border-[#444]/50 pl-[28px] pr-[16px]">
                                <span className="text-[15px] font-bold text-[#86868B] tracking-tight shrink-0">건폐율</span>
                                <div className="relative group/tooltip min-w-0 flex items-center">
                                    <span className="text-[16px] text-[#E5E5E5] tracking-tight font-medium truncate pr-4 cursor-default w-full">47.02%</span>
                                    <div className="absolute bottom-[100%] left-0 mb-2 px-3 py-1.5 bg-[#333] text-[#E5E5E5] text-[12px] font-medium rounded-md whitespace-nowrap opacity-0 group-hover/tooltip:opacity-100 transition-opacity duration-200 pointer-events-none z-50 shadow-lg border border-[#444]">
                                        47.02%
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-[84px_1fr] items-center border-b border-[#444]/50 pl-[24px] pr-[32px]">
                                <span className="text-[15px] font-bold text-[#86868B] tracking-tight shrink-0">개방형녹지</span>
                                <div className="relative group/tooltip min-w-0 flex items-center">
                                    <span className="text-[16px] text-[#E5E5E5] tracking-tight font-medium truncate cursor-default w-full">TBD</span>
                                    <div className="absolute bottom-[100%] left-0 mb-2 px-3 py-1.5 bg-[#333] text-[#E5E5E5] text-[12px] font-medium rounded-md whitespace-nowrap opacity-0 group-hover/tooltip:opacity-100 transition-opacity duration-200 pointer-events-none z-50 shadow-lg border border-[#444]">
                                        TBD
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-[84px_1fr] items-center border-r border-[#444]/50 pl-[28px] pr-[16px]">
                                <span className="text-[15px] font-bold text-[#86868B] tracking-tight shrink-0">용적률</span>
                                <div className="relative group/tooltip min-w-0 flex items-center">
                                    <span className="text-[16px] text-[#E5E5E5] tracking-tight font-medium truncate pr-4 cursor-default w-full">1,056.62%</span>
                                    <div className="absolute bottom-[100%] left-0 mb-2 px-3 py-1.5 bg-[#333] text-[#E5E5E5] text-[12px] font-medium rounded-md whitespace-nowrap opacity-0 group-hover/tooltip:opacity-100 transition-opacity duration-200 pointer-events-none z-50 shadow-lg border border-[#444]">
                                        1,056.62%
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-[84px_1fr] items-center pl-[24px] pr-[32px]">
                                <span className="text-[15px] font-bold text-[#86868B] tracking-tight shrink-0">전용률</span>
                                <div className="relative group/tooltip min-w-0 flex items-center">
                                    <span className="text-[16px] text-[#E5E5E5] tracking-tight font-medium truncate cursor-default w-full">TBD</span>
                                    <div className="absolute bottom-[100%] left-0 mb-2 px-3 py-1.5 bg-[#333] text-[#E5E5E5] text-[12px] font-medium rounded-md whitespace-nowrap opacity-0 group-hover/tooltip:opacity-100 transition-opacity duration-200 pointer-events-none z-50 shadow-lg border border-[#444]">
                                        TBD
                ...
                <div className="w-full bg-[#292928] border border-[#3c3c3c] rounded-[32px] flex flex-col mb-[20px] overflow-hidden">
                    ...
                    <div className="pl-[30px] pr-[32px] py-[24px] h-[106px] flex flex-col relative w-full items-center justify-center">
                        <div className="absolute top-[28px] left-[30px]">
                            <span className="text-[14px] font-bold text-[#86868B] tracking-tight">Hotel</span>
                        </div>
                        <span className="text-[#86868B] font-bold text-[24px]">TBD</span>
                    </div>
                    ...
                    <div className="w-full h-[54px] border-t border-[#444]/50 flex items-center pl-[30px] gap-[12px] shrink-0">
                        <span className="text-[13px] font-bold text-[#86868B]">호텔 담당</span>
                    ...
                    <div className="flex flex-row w-full flex-1">
                        ...
                        <div className="w-[380px] flex flex-col shrink-0">
                            ...
                            <div className="pl-[30px] pr-[32px] pt-[28px] pb-[27px] flex flex-col flex-1 relative">
                                ...
                                {/* Content */}
                                <div className="flex flex-col gap-[8px]">
                                    {ecoSpecs.length > 0 ? ecoSpecs.map(item => (
                                        ...
                                        <div className="text-[15px] leading-[22px] text-[#A1A1AA]">TBD</div>
                                    )}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 flex flex-col">
                            ...
                            <div className="pr-[32px] pt-[28px] pb-[27px] flex flex-col flex-1 relative">
                                ...
                                <div className="pl-[30px] border-l border-[#444]/50 flex flex-col gap-[8px]">
                                    {buildingSpecs.length > 0 ? buildingSpecs.map(item => (
                                        <div key={item.id} className="text-[15px] leading-[22px]">
                                            <span className="text-[#86868B] font-medium mr-[6px] tracking-tight">{item.label}</span>
                                            <div className="relative inline-block group/link cursor-pointer">
                                                <span onClick={(e) => e.preventDefault()} className="text-[#c3c2b7] font-medium tracking-tight hover:text-[#fbf167] transition-colors">
                ...
                {comparisonData.map((data) => (
                    ...
                    <div className="flex-1 bg-[#292928] border border-[#3c3c3c] rounded-[32px] overflow-hidden px-[32px] pt-[24px] pb-[32px] flex flex-col h-[360px]">
                        ...
                        <div className="flex items-center justify-between w-full pb-[24px]">
                            ...
                            >
                                <span className="text-[13px] font-medium tracking-tight">IOTA Seoul 1 427과 비교하기</span>
